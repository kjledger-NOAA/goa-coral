...
```{r}
path <- "~/Documents/goa-coral/fastq/20260520_GOAcoral_mifish/trimmed/"
...
```{r}
# Retain only non-zero read samples 
nonzero <- out[, "reads.out"] > 0
filtFs <- filtFs[nonzero]
filtRs <- filtRs[nonzero]
...
track <- cbind(out[nonzero,], sapply(dadaFs, getN), sapply(dadaRs, getN), sapply(mergers, getN), rowSums(seqtab2), rowSums(seqtab.nochim))
# If processing a single sample, remove the sapply calls: e.g. replace sapply(dadaFs, getN) with getN(dadaFs)
colnames(track) <- c("input", "filtered", "denoisedF", "denoisedR", "merged", "correct_length","nonchim")
rownames(track) <- sample.names[nonzero]
head(track)
```
...
out_path <- file.path(here("data", "20260520_mifish_outputs"))
